...
const ShipmentInfo = ({ shipments }) => {
  ...
  return (
    <div>
      ...
      {shipments.data && shipments.loaded && (
        <div key={shipments.data.id}>
          <span>Shipment No. {shipments.data.trackingNumber}</span>
          <h2>{shipments.data.state}</h2>
          <p>
            {shipments.data.state === 'DELIVERED' ? 'Your shipper requested a pickup. Bosta will pick it up soon' : 'Order is canceled and it will be returned back to the shipper'}
          </p>
        </div>
      )}
    </div>
